...
def execute_llm_pipeline(user_message: str, user_id: str) -> str:
    messages = [
        {
            "role": "system",
            "content": (
    ...
    # Fetch recent chat history for this specific user
    try:
...
def persist_to_db(user_msg: str, ai_msg: str, user_id: str):
    try:
...
def chat(req: ChatRequest, user_id: str = Depends(decode_token)):
    ai_reply = execute_llm_pipeline(req.message, user_id)
    persist_to_db(req.message, ai_reply, user_id)
...
async def chat_audio(file: UploadFile = File(...), user_id: str = Depends(decode_token)):
    try:
        ...
        ai_reply = execute_llm_pipeline(user_message, user_id)
        persist_to_db(user_message, ai_reply, user_id)
